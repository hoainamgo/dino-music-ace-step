---
name: vi-short-dub-vilao
description: Dub an English short video (≤60s) into Vietnamese with Whisper ASR local + Kokoro-Vietnamese TTS + per-segment atempo sync + Vilao OCR verify. English audio kept at 37% bg. Use when user wants a single short translated ASR→TTS without an external translation API.
